...
In order to reach a more efficient model we used re-code_factor() argument which requires the dplyr R package to be loaded, and the function takes the original factor variable as the first argument like main mode, followed by the re-coding rules and then combines car and motor into private mode, taxi, collective, metro, and bus into public mode and bicycle and walk into active mode of transportation. The re-coding rules in this study are specified using a set of named character vectors to aggregate or disperse the original levels of the factor and the values represent the new levels.
...
In this research, our dependent variables which are stress experienced during commute and the importance of the negative feeling assigned by commuters contribute to the coping strategies used by commuters.
...
mod_beta <- data.frame(Equation1 = c(NA,
                       ...
                       Equation2 = c("IC_AgeAdults 1",
                                ...
                                NA,
                                NA,
                                "IC_EducationHigher Education 1",
                                NA,
                                NA),
                                
                              
                        beta_equation2 = c (mod_bivariate[["beta"]][["IC_AgeAdults 1"]],
                                            NA,
                                            NA,
                                            ...
                                            NA,
                                            NA,
                                            mod_bivariate[["beta"]][["IC_EducationHigher Education 1"]],
                                            NA,
                                            NA),
                        
                        
                        se_equation2 = c( mod_bivariate[["sebeta"]][["IC_AgeAdults 1"]],
                                         NA,
                                         NA,
                                         ...
                                         NA,
                                         NA,
                                         mod_bivariate[["sebeta"]][["IC_EducationHigher Education 1"]],
                                         NA,
                                         NA),
...
df|>
  filter(!is.na(H_Imp_stress), !is.na(IC_Income), !is.na(IC_Income)) |>
  ggplot() +
  geom_mosaic(aes(x = product(H_Imp_stress, IC_Income), fill = H_Imp_stress)) +
  theme(axis.text = element_text(angle = 90, size = 5)) +
  facet_wrap(~IC_Age)+
